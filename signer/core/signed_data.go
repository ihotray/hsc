...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	...
	case apitypes.ApplicationBlockbuster.Mime:
		stringData, ok := data.(string)
		if !ok {
			return nil, useEthereumV, fmt.Errorf("input for %v must be an hex-encoded string", apitypes.ApplicationBlockbuster.Mime)
		}
		blockbusterData, err := hexutil.Decode(stringData)
		if err != nil {
			return nil, useEthereumV, err
		}
		header := &types.Header{}
		if err := rlp.DecodeBytes(blockbusterData, header); err != nil {
			return nil, useEthereumV, err
		}
		// The incoming blockbuster header is already truncated, sent to us with a extradata already shortened
		if len(header.Extra) < 65 {
			// Need to add it back, to get a suitable length for hashing
			newExtra := make([]byte, len(header.Extra)+65)
			copy(newExtra, header.Extra)
		...
		// Get back the rlp data, encoded by us
		sighash, blockbusterRlp, err := blockbusterHeaderHashAndRlp(header, api.chainID)
		if err != nil {
			return nil, useEthereumV, err
		}
		messages := []*apitypes.NameValueType{
			{
				Name:  "Blockbuster header",
				Typ:   "blockbuster",
				Value: fmt.Sprintf("blockbuster header %d [0x%x]", header.Number, header.Hash()),
			},
		}
		// Blockbuster uses V on the form 0 or 1
		useEthereumV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: blockbusterRlp, Messages: messages, Hash: sighash}
...
func blockbusterHeaderHashAndRlp(header *types.Header, chainId *big.Int) (hash, rlp []byte, err error) {
	if len(header.Extra) < 65 {
		err = fmt.Errorf("blockbuster header extradata too short, %d < 65", len(header.Extra))
		return
	}
	rlp = blockbuster.BlockbusterRLP(header, chainId)
	hash = blockbuster.SealHash(header, chainId).Bytes()
	return hash, rlp, err
}
