...
type Snapshot struct {
	config   *params.BlockbusterConfig // Consensus engine parameters to fine tune behavior
...
// the genesis block.
func newSnapshot(
	config *params.BlockbusterConfig,
...
func loadSnapshot(config *params.BlockbusterConfig, sigCache *lru.ARCCache, db ethdb.Database, hash common.Hash, ethAPI *ethapi.PublicBlockChainAPI) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("blockbuster-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(Snapshot)
...
func (s *Snapshot) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("blockbuster-"), s.Hash[:]...), blob)
}
