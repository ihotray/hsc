...
func InspectDatabase(db ethdb.Database, keyPrefix, keyStart []byte) error {
	...
	for it.Next() {
		...
		case bytes.HasPrefix(key, []byte("blockbuster-")) && len(key) == 12+common.HashLength:
			blockbusterSnaps.Add(size)
	...
	stats := [][]string{
		...
		{"Key-Value store", "Blockbuster snapshots", blockbusterSnaps.Size(), blockbusterSnaps.Count()},
